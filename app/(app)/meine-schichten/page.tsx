...
function LiveView({
  ...
  // Besetzung der jeweils tatsächlich eingeplanten Schicht – nicht mehr über
  // eine feste Schichtzuordnung, die es im Rotationsbetrieb gar nicht gibt.
  // Der Plan liefert pro Tag die effektive Schicht (Ausnahme > Muster >
  // feste Zuordnung), dazu holen wir die Besetzung.
  useEffect(() => {
    ...
    let cancelled = false;
    const workdays = plan
      .filter((d) => !d.isFree && d.shiftId && !isHoliday(d.date, holidaysReal))
      .slice(0, 14);
    ...
    Promise.all(workdays.map((d) => fetchShiftStaffing(d.shiftId!, d.date)))
      .then((results) => {
        if (!cancelled) {
          setTwoWeeks(results.filter((r): r is LiveStaffingSnapshot => r !== null));
        }
      })
      .catch(() => {
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader
              title="Nächste Arbeitstage"
              hint="Besetzung der jeweils eingeplanten Schicht"
            />
            <CardBody className="space-y-1.5">
              {twoWeeks === null ? (
                <p className="px-2 py-4 text-sm text-ink-muted">wird geladen …</p>
                ...
                twoWeeks.map((snap) => (
                  ...
                  >
                    <span className="min-w-0">
                      <span className="tnum block text-ink-muted">
                        {WEEKDAY_SHORT[(fromISO(snap.date).getDay() + 6) % 7]} ·{" "}
                        {formatDE(snap.date)}
                      </span>
                      {snap.shiftName ? (
                        <span className="block text-[12px] text-ink-faint">{snap.shiftName}</span>
                      ) : null}
                    </span>
                    <span className="flex items-center gap-3">
                      <span className="tnum font-medium">
